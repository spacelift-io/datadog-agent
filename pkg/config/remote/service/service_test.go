...
func TestServiceBackoff(t *testing.T) {
	...
	// should be somewhere in the range of 1 + [1,2], so [2,3]
	assert.Equal(t, service.backoffErrorCount, 1)
	refreshInterval := service.calculateRefreshInterval()
	assert.GreaterOrEqual(t, refreshInterval, 2*time.Second)
	assert.LessOrEqual(t, refreshInterval, 3*time.Second)

	err = service.refresh()
	assert.NotNil(t, err)

	// Now we're looking at  1 + [2, 4], so [3,5]
	assert.Equal(t, service.backoffErrorCount, 2)
	refreshInterval = service.calculateRefreshInterval()
	assert.GreaterOrEqual(t, refreshInterval, 3*time.Second)
	assert.LessOrEqual(t, refreshInterval, 5*time.Second)

	err = service.refresh()
	assert.NotNil(t, err)

	// After one more we're looking at  1 + [4, 8], so [5,9]
	assert.Equal(t, service.backoffErrorCount, 3)
	refreshInterval = service.calculateRefreshInterval()
	assert.GreaterOrEqual(t, refreshInterval, 5*time.Second)
	assert.LessOrEqual(t, refreshInterval, 9*time.Second)
	...
	// Our recovery interval is 2, so we step back to the [2,3] range
	assert.Equal(t, service.backoffErrorCount, 1)
	refreshInterval = service.calculateRefreshInterval()
	assert.GreaterOrEqual(t, refreshInterval, 2*time.Second)
	assert.LessOrEqual(t, refreshInterval, 3*time.Second)
