...
// Containt values that control the backoff strategy the client will use
const (
	minimalMaxBackoffTime = 120 * time.Second
	maximalMaxBackoffTime = 300 * time.Second
)
...
func NewService() (*Service, error) {
	...
	//
	// The following values mean each range will always be [2^<NumErrors-1>, min(maxBackoffTime, 2^<NumErrors>)].
